...
app.get("/",function(req,res){
  res.render('home',{
    ...
    newPost: composeDetails});
  

});
app.get('/post/:testing', (req, res) => {
  const routedata = _.lowerCase(req.params.testing);
  composeDetails.forEach(function(composeDetail){

    const storedTitle = _.lowerFirst(composeDetail.postPost);

    if(routedata===storedTitle){
      res.render("post",{
        newheading : composeDetail.postPost,
        newcontent : composeDetail.postTitle
      })
...
app.post("/compose",function(req,res){
  const post = {
    postPost : req.body.journalPost,
    postTitle : req.body.journalTitle
  };
  composeDetails.push(post);
  res.redirect("/")
